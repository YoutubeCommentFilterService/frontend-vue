...
const visiblePages = computed(() => {
  ...
  if (props.totalPages <= MAX_LEFT_RIGHT_NUM * 2 + 1) {
    ...
    if (currentPage.value <= MAX_LEFT_RIGHT_NUM * 2) {
      for (let i = 1; i <= currentPage.value + MAX_LEFT_RIGHT_NUM; i++) pages.push(i)
      pages.push('...', props.totalPages)
    } else if (currentPage.value >= props.totalPages - (MAX_LEFT_RIGHT_NUM * 2 - 1)) {
      pages.push(1, '...')
      for (let i = currentPage.value - MAX_LEFT_RIGHT_NUM; i <= props.totalPages; i++) pages.push(i)
    } else {
      pages.push(1, '...')
      for (
        let i = currentPage.value - MAX_LEFT_RIGHT_NUM;
        i <= currentPage.value + MAX_LEFT_RIGHT_NUM;
        i++
      )
        pages.push(i)
      pages.push('...', props.totalPages)
